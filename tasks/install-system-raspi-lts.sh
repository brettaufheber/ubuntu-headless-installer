...
function install_system_raspi_lts {
  ...
  if [[ -n "${POST_INSTALL_CMD:-}" ]]; then
    chroot "$CHROOT" /bin/bash -c "$POST_INSTALL_CMD"
  fi
...
function mount_image {
  ...
  # create directories required for the system root mount points
  mkdir -p "$IMAGE_ROOT"
...
function configure_boot {
  ...
  if echo "$CMD" | grep -q 'root='; then
    CMD=$(echo "$CMD" | sed -E "s/root=[^ ]+/root=$UUID_ROOT/")
  else
    CMD="$CMD root=$UUID_ROOT"
  fi

  if echo "$CMD" | grep -q 'rootfstype='; then
